...
#include "LT.h"
using namespace std;
#define MFST_TRACE_START(log) *log.stream << setw(4) <<  std::setfill(' ') << left << "Шаг:"\
							  << setw(20) << left << " Правило" \
							  << setw(30) << left << " Входная лента" \
							  << setw(20) << left << " Стек" << endl;
#define MFST_TRACE1(stream)  stream<< setw(4) << left <<  std::setfill(' ')<< ++FST_TRACE_n << ": "\
...
namespace MFST
{
	...
	struct Mfst
	{
		...
		bool savestate(ofstream* stream);
		bool reststate(ofstream* stream);
		bool push_chain(GRB::Rule::Chain chain);
		RC_STEP step(ofstream* stream);
		bool start(ofstream* stream);
		bool savediagnosis(RC_STEP pprc_step);
		void printrules(ofstream* stream);
